...
public class ControllerAdviceBeanDefinition extends DefaultBeanDefinition {
    ...
    public ControllerAdviceBeanDefinition(String profileStr, Class<?> klass) {
    ...
    protected void analysisMethodByAnnotation(Class<? extends Annotation> annotationType, Annotation annotation, Method method) {
        // 异常处理器
        if (annotationType == ExceptionHandler.class) {
    ...
    private void doAnalysisExceptionHandler() {
        for (ExceptionMethodDefinition emd : exceptionMethodList) {
            ExceptionHandlerSelector.registerExceptionHandler(new ExceptionMethodWrapper(single, emd));
        }
    }
}