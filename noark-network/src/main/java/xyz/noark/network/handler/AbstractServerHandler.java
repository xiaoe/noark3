...
public abstract class AbstractServerHandler<T> extends SimpleChannelInboundHandler<T> {
    ...
    protected void dispatchPacket(ChannelHandlerContext ctx, NetworkPacket packet) {
        try (ByteArray ignored = packet.getByteArray()) {
            ...
            // 开启了数据统计功能.
            if (receiveActive && !this.statPacket(session, packet)) {
                return;
            }
     ...
     * 流量统计
     */
    private boolean statPacket(Session session, NetworkPacket packet) {
        ...
        if (packetLength >= receiveThreshold) {
            ...
            if (warnCount >= receiveCount) {
                ...
                if (networkListener != null) {
                    return networkListener.handlePacketWarning(session, receiveSecond, warnCount, receiveThreshold);
                }
            }
        }
        return true;
